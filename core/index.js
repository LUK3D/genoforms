...
function getTablesFromDatabase(database, _callback) {
    if (auth.env == "mssql") {
        ...
        auth.run("SELECT table_name FROM information_schema.tables WHERE table_schema = '" + database + "'", function (results) {
            _callback(results);
        });
    }
}
 ...
 * @param _callback Callback function that will be fired with the set of tables as argument
 */
function getProceduresFromDatabase(database, _callback) {
    auth.run("SELECT SPECIFIC_CATALOG, SPECIFIC_NAME, ROUTINE_NAME,ROUTINE_DEFINITION  FROM " + database + ".INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE'", function (results) {
        _callback(results.recordset);
    });
}
/**
...
setTimeout(function () {
    ...
    var finalSql = [];
    getTablesFromDatabase("MASI_TESTES", function (tables) {
